...
const StatisticsView: React.FC<StatisticsViewProps> = ({ reports, employees }) => {
  ...
  const stats = useMemo(() => {
    ...
    const disciplineSummaries = summarizeAllEmployeesDiscipline(employees, reports);
    const actionRequired = disciplineSummaries.filter(item => item.totalMinutes >= 120).slice(0, 20);
